...
{        
    ...
    if (ib==0) { 
        ...
        GetFaceNodes(tmp.tempn, sol.dudg, mesh.facecon, npf, ncu, npe, nc, f1, f2, 0, backend);
        PutElemNodes(sol.duh, tmp.tempn, npf, ncu, 0, ncu, f1, f2, backend);
    } 
    else {
        ...
        GetArrayAtIndex(&tmp.tempg[n4], sol.udg, &mesh.findudg1[npf*nc*f1], nn*nc, backend);
        // v = dudg = tmp.tmpg[n8]
        GetArrayAtIndex(&tmp.tempg[n8], sol.dudg, &mesh.findudg1[npf*nc*f1], nn*nc, backend);

        // TODO: need to decide whether to set to 0 out here or inside drivers
        ArraySetValue(tmp.tempn, zero, 2*nn*ncu, backend);
        ...
        PutElemNodes(sol.duh, &tmp.tempn[nn*ncu], npf, ncu, 0, ncu, f1, f2, backend);                  
    }
}
